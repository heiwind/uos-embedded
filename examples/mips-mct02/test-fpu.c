...
void console (void *arg)
{
	...
	for (;;) {
		printf (&uart, "\33[H");
		printf (&uart, "Testing FPU.\n\n");

		printf (&uart, "     Duration: %02u:%02u:%02u \n", timer_hour, timer_min, timer_sec);
		printf (&uart, "CPU frequency: %u MHz\n", KHZ / 1000);
		printf (&uart, "  SDRAM clock: %u MHz\n", MPORT_KHZ / 1000);
		printf (&uart, "   Reg.CSCON3: %08x  \n", MC_CSCON3);
#ifdef ENABLE_ICACHE
		printf (&uart, "  Instr.cache: enabled  \n");
#else
		printf (&uart, "  Instr.cache: disabled  \n");
#endif
#ifdef ENABLE_DCACHE
		printf (&uart, "   Data cache: enabled  \n");
#else
		printf (&uart, "   Data cache: disabled  \n");
#endif
		printf (&uart, "\n");
		printf (&uart, "---------Value-----------------Loops-------Errors----\n");
		printf (&uart, "     e:  %-20.15f  %-10u  %u \n", e, count_e_loops, count_e_errors);
		printf (&uart, "Machin:  %-20.15f  %-10u  %u \n", pi_machin, count_machin_loops, count_machin_errors);
		printf (&uart, " Gauss:  %-20.15f  %-10u  %u \n", pi_gauss, count_gauss_loops, count_gauss_errors);
		printf (&uart, " Brent:  %-20.15f  %-10u  %u \n", pi_brent, count_brent_loops, count_brent_errors);
