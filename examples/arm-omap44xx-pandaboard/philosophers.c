...
void display (int i, state_t state)
{
	...
	debug_printf ("\033[%d;%dH", ypos [i], xpos [i]);
	switch (state) {
	case THINKING:
		debug_printf ("\033[1;34m%d: thinking", i+1);	/* Light blue */
		break;
	case EATING:
		debug_printf ("\033[1;32m%d: eating  ", i+1);	/* Light green */
		break;
	case WAITING:
		debug_printf ("\033[1;31m%d: waiting ", i+1);	/* Light red */
		break;
	}
	mutex_unlock (&screen);
}
...
void uos_init ()
{
	...
	timer_init (&timer, SYSTIMER_KHZ, 50);
