...
#define TIMER_IRQ	7	/* Прерывание от таймера */
...
static bool_t timer_handler (void *arg)
{
	...
	unsigned latency = mips_read_c0_register (C0_COUNT) - compare;
	do {
...
void uos_init (void)
{
	...
	/* Используем системный таймер. */
	unsigned count = mips_read_c0_register (C0_COUNT);
	compare_step = (KHZ * MSEC_PER_TICK + 1) / 2;
	mips_write_c0_register (C0_COMPARE, count + compare_step);
