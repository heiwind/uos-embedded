...
void do_infrared (unsigned char c)
{
	...
	case IRRPT_PLAY:
		...
		motor_set_power(&motor, MOTOR_LEFT, 127);
		motor_set_power(&motor, MOTOR_RIGHT, 127);
		power_print ();
		break;
	case IRCMD_STOP:
		/* левый, правый стоп */
	...
	case IRCMD_REW:
	case IRRPT_REW:
		/* левый назад, а правый вперед на 1/3 макс (Налево) */
		motor_set_power(&motor, MOTOR_LEFT, -42);
		motor_set_power(&motor, MOTOR_RIGHT, 42);
		power_print ();
		break;
	case IRCMD_FF:
	case IRRPT_FF:
		/* левый вперед, а правый назад на 1/3 макс (Направо) */
		motor_set_power(&motor, MOTOR_LEFT, 42);
		motor_set_power(&motor, MOTOR_RIGHT, -42);
		power_print ();
		break;
