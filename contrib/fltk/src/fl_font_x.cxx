//
// "$Id: fl_font_x.cxx,v 1.4 2008-05-05 15:25:59 vak Exp $"
//
...
static Fl_FontSize* find(int fnum, int size) {
  ...
  if (!s->xlist) {
    ...
    if (!s->xlist) {	// use variable if no matching font...
      s->first = new Fl_FontSize("variable");
      s->first->minsize = 0;
  ...
  for (int n=0; n < m; n++) {
    ...
    if (thissize == size && thislength < matchedlength) {
      ...
      l += snprintf(namebuffer+l,sizeof(namebuffer)-l,"%d",size);
      while (*c == '0') c++;
...
//
// End of "$Id: fl_font_x.cxx,v 1.4 2008-05-05 15:25:59 vak Exp $".
//
