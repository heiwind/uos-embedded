//
// "$Id: Fl_GIF_Image.cxx,v 1.4 2008-05-05 15:25:59 vak Exp $"
//
...
Fl_GIF_Image::Fl_GIF_Image(const char *infname) : Fl_Pixmap((char *const*)0) {
  ...
  (void) NEXTBYTE; // Background Color index
  (void) NEXTBYTE; // Aspect ratio is N/64
  ...
  for (;;) {
	...
	char bits;
	bits = NEXTBYTE;
	(void) NEXTBYTE; (void) NEXTBYTE; // GETSHORT(delay);
	...
	Fl::warning("%s: unknown gif extension 0x%02x.", infname, ch);
      ...
      (void) NEXTBYTE; (void) NEXTBYTE; // GETSHORT(x_position);
      (void) NEXTBYTE; (void) NEXTBYTE; // GETSHORT(y_position);
      GETSHORT(Width);
      GETSHORT(Height);
      ch = NEXTBYTE;
      Interlace = ((ch & 0x40) != 0);
    ...
    // skip the data:
    while (blocklen>0) {while (blocklen--) {(void)NEXTBYTE;} blocklen=NEXTBYTE;}
  }
...
//
// End of "$Id: Fl_GIF_Image.cxx,v 1.4 2008-05-05 15:25:59 vak Exp $".
//
