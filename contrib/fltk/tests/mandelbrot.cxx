//
// "$Id: mandelbrot.cxx,v 1.3 2005-09-07 10:50:18 vak Exp $"
//
...
static void window_callback(Fl_Widget*, void*) {
	debug_printf ("mandelbrot callback\n");
//	exit(0);
}

extern "C" int nxmain (void *arg) {
  int argc = 1;
  char *argv[] = {"a.out",0};
  mbrot.make_window();
  mbrot.d->X = -.75;
...
void Drawing_Window::update_label() {
  char buffer[128];
  snprintf(buffer, sizeof(buffer), "%+.10f", d->X); x_input->value(buffer);
  snprintf(buffer, sizeof(buffer), "%+.10f", d->Y); y_input->value(buffer);
  snprintf(buffer, sizeof(buffer), "%.2g", d->scale); w_input->value(buffer);
}
...
int Drawing_Area::handle(int event) {
  ...
  case FL_RELEASE:
	...
	jbrot.update_label();
      ...
      static char s[128];
      snprintf(s, sizeof(s), "Julia %.7f %.7f",jbrot.d->jX,jbrot.d->jY);
...
//
// End of "$Id: mandelbrot.cxx,v 1.3 2005-09-07 10:50:18 vak Exp $".
//
