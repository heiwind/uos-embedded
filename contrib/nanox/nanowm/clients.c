...
#include "nanox/include/nxdraw.h"
...
int Wm_new_client_window(GR_WINDOW_ID wid)
{
	...
	if (style & GR_WM_PROPS_NOAUTOMOVE) {
		...
		x = lastx + WINDOW_STEP;
		if((x + width) > Wm_si.cols)
			x = FIRST_WINDOW_LOCATION;
		lastx = x;
		y = lasty + WINDOW_STEP;
		if((y + height) > Wm_si.rows)
	...
	pid = GrNewWindow(GR_ROOT_WINDOW_ID, x, y, width, height,
		0, GR_COLOR_GREY75, GR_COLOR_BLACK);
	window.wid = pid;
	...
	nid = GrNewWindow(pid, 0, 0, TITLE_BAR_HEIGHT, TITLE_BAR_HEIGHT, 0,
		GR_COLOR_GREY75, GR_COLOR_BLACK);
	window.wid = nid;
	window.pid = pid;
	...
	nid = GrNewWindow(pid, TITLE_BAR_HEIGHT + 1, 1, width - (4 *
		TITLE_BAR_HEIGHT) - 3, TITLE_BAR_HEIGHT - 3, 1,
		GR_COLOR_GREY75, GR_COLOR_BLACK);
	window.wid = nid;
	window.pid = pid;
	...
	nid = GrNewWindow(pid, width - (3 * TITLE_BAR_HEIGHT), 0,
		TITLE_BAR_HEIGHT, TITLE_BAR_HEIGHT, 0,
		GR_COLOR_GREY75, GR_COLOR_BLACK);
	window.wid = nid;
	window.pid = pid;
	...
	nid = GrNewWindow(pid, 1, TITLE_BAR_HEIGHT + 1, BORDER_WIDTHS - 2,
		height - TITLE_BAR_HEIGHT - BORDER_WIDTHS - 1, 1,
		GR_COLOR_GREY75, GR_COLOR_BLACK);
	window.wid = nid;
	window.pid = pid;
	...
	nid = GrNewWindow(pid, 1, height - BORDER_WIDTHS + 1, BORDER_WIDTHS - 2,
		BORDER_WIDTHS - 2, 1, GR_COLOR_GREY75, GR_COLOR_BLACK);
	window.wid = nid;
	window.pid = pid;
	...
	nid = GrNewWindow(pid, width - BORDER_WIDTHS + 1,
		TITLE_BAR_HEIGHT + 1, BORDER_WIDTHS - 2,
		height - TITLE_BAR_HEIGHT - BORDER_WIDTHS - 1,  1,
		GR_COLOR_GREY75, GR_COLOR_BLACK);
	window.wid = nid;
	window.pid = pid;
...
void Wm_client_window_remap(win *window) {
  ...
  if(!(pwin = Wm_find_window(window->pid))) {
    debug_printf ("Couldn't find parent of destroyed window "
	    "%d\n", window->wid);
    return;
  }
...
Wm_client_window_resize(win *window)
{
	...
	if(!(pwin = Wm_find_window(window->pid))) {
		debug_printf ("Couldn't find parent of resize window %d\n", window->wid);
		return;
	}
