...
setupGC(Display * dpy, GC gc, unsigned long valuemask, XGCValues * values)
{
	...
	if (valuemask & GCPlaneMask)
		debug_printf("XCreatGC: GCPlaneMask not implemented\n");

	if (valuemask & GCArcMode)
		debug_printf("XCreatGC: GCArcMode not implemented\n");
}
...
{
	debug_printf("XFlushGC called, not synced\n");
}
...
//debug_printf("XSetForeground clr %x pix %x\n", (int)c, (int)foreground);
	return 1;
}
...
{
	if (dash_offset)
		debug_printf("XSetDashes: dash offset not implemented\n");
