...
//#define DEBUG
#ifdef DEBUG
#define FUNC_ENTER debug_printf("ENTER [%s]\n", __FUNCTION__)
#define FUNC_EXIT debug_printf("LEAVE [%s]\n", __FUNCTION__)
#else
#define FUNC_ENTER
#define FUNC_EXIT
#endif
...
translateNXEvent(Display *dpy, GR_EVENT * ev, XEvent * event)
{
	...
	case GR_EVENT_TYPE_CHLD_UPDATE:
		{
			...
			} else if (pev->utype == GR_UPDATE_ACTIVATE) {
				/* Do nothing. */
			} else
				debug_printf("translateNXEvent: unhandled update event subtype %d\n", pev->utype);
			break;
		}
	...
	case GR_EVENT_TYPE_FOCUS_OUT:
		...
		break;
	default:
		debug_printf("translateNXEvent: UNHANDLED EVENT %d\n", ev->type);
	}
}
...
{
	if (saved) {
		debug_printf("XPutBackEvent: lost event\n");
		return 0;
	}
	saved = 1;
	saved_event = *event;
...
#ifdef DEBUG
	if (ret)
		debug_printf("Returning %d events ready\n", ret);
#endif
	return ret;
}
