...
#if UOS
/* Use embedded rgb.txt database. */

struct rgbname {
	unsigned char r, g, b;
	const char *colorname;
};
static const struct rgbname rgbtab [] = {
#include "rgbtxt.h"
};

static int rgbcompare (const void *colorname, const void *elem)
{
	int ret;
...
GrGetColorByName (char *colorname, int *retr, int *retg, int *retb)
{
	...
	rgb = bsearch (colorname, rgbtab, sizeof(rgbtab) / sizeof(rgbtab[0]),
		sizeof(rgbtab[0]), rgbcompare);
	if (! rgb)
		return 0;
	...
	if (retb)
		*retb = rgb->b;
	return GR_RGB (rgb->r, rgb->g, rgb->b);
}
