...
static void init (nxeyes_state *state)
{
	...
	state->mouse_moved = 1;
	state->gc = GrNewGC();
	GrSetGCForeground(state->gc, GR_COLOR_YELLOW);
	GrSetGCBackground(state->gc, GR_COLOR_BLACK);

	state->wid = GrNewWindowEx (GR_WM_PROPS_NODECORATE, (unsigned char*) "",
		GR_ROOT_WINDOW_ID, 0, 0, (EYEFG_HEIGHT * 2) + EYE_SPACING,
		EYEFG_HEIGHT, GR_COLOR_YELLOW);
...
void start_blink_timer(nxeyes_state *state)
{
	...
	if (state->eyes_closed)
		time = BLINK_CLOSED_TIME;
	else
		time = (rand15() % (BLINK_MAX_OPEN_TIME - BLINK_MIN_OPEN_TIME)) +
			BLINK_MIN_OPEN_TIME;
