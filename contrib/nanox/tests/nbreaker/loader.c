...
#else
/* Store embedded gamefile as an array of strings. */
#include "gamefile.h"
...
static int parse_rows(nbstate *state, level *lev, FILE *fp, int *line)
{
	...
	while(fgets(buf, 256, fp)) {
		...
		*line = *line + 1; /* increment the line number. */
		...
		if(!(p = strchr(buf, '\n'))) {
			...
			debug_printf ("Too long line on line %d of game file "
					"\"%s\"\n", *line, state->gamefile);
			return 1;
		}
		*p = 0; /* Get rid of the newline. */
		...
		if(y == state->height) {
			...
			debug_printf ("Too many rows in level definition on"
					"line %d of game file \"%s\"\n",
					*line, state->gamefile);
			return 1;
		}
		...
		for(p = buf; *p; p++) {
			...
			if(++x > state->width) {
				debug_printf ("Error: row too long on line "
			...
			if(*p == ' ') b = 0; /* A space means "no brick". */
			else {
				...
				if(!b) {
					 ...
					 * "failure": */
					debug_printf ("Error: undefined brick"
						" \"%c\" on line %d of game "
						"file \"%s\"\n", *p, *line,
						state->gamefile);
					return 1;
	...
	if(feof(fp)) {
		debug_printf ("Error: premature end of file inside rows "
				"block that started on line %d of game file "
				"\"%s\"\n", startline, state->gamefile);
	} else debug_printf ("Error reading from game file \"%s\"\n",
			state->gamefile);
	return 1;
}
...
static int parse_powers(nbstate *state, level *lev, FILE *fp, int *line)
{
	...
	while(fgets(buf, 256, fp)) {
		...
		*p = 0; /* Get rid of the newline. */
		...
		for(p = buf; *p; p++) {
			 ...
			 * return "failure": */
			if(!powers[i]) {
				debug_printf ("Error: invalid power \"%c\" "
	 ...
	 * "failure" to the caller: */
	if(feof(fp)) {
		debug_printf ("Error: premature end of file inside powers "
				"block that started on line %d of game file "
				"\"%s\"\n", startline, state->gamefile);
	} else debug_printf ("Error reading from game file \"%s\"\n",
			state->gamefile);
	...
	unsigned char *buf)
{
	...
	if(b) {
		debug_printf ("Warning: ignoring duplicate brick definition "
				"on line %d of game file \"%s\"\n", line,
				state->gamefile);
		return 0;
	}
	...
	if(flags) {
		...
		while((f = *flags++)) {
			...
			if(f == 'I') b->flags |= BRICK_FLAG_IMMUTABLE;
				...
				debug_printf ("Warning: ignoring invalid "
	...
	if(i > 1) {
		debug_printf ("Warning: brick flags I, 2, and 3 are mutually "
	 ...
	 * and cancel all of them: */
	if(i > 1) {
		debug_printf ("Warning: brick flags S, M, L, and H are "
	...
	if(!(b->s = load_sprite(state, buf, state->brickwidth,
		...
		if(!(b->s = make_empty_sprite(state, buf, state->brickwidth,
			 ...
			 * return "failure": */
			debug_printf ("Error: failed to create brick "
		...
		GrSetGCForeground(state->gc, GR_COLOR_NAVYBLUE);
...
static int parse_powersprite(nbstate *state, unsigned char *buf, int line)
{
	 ...
	 * return "failure": */
	if(power == NOPOWER) {
		debug_printf ("Invalid power \"%c\" on line %d of game file "
	...
	if(!(s = load_sprite(state, buf + 2, -1, -1))) {
		...
		if(!(s = make_empty_sprite(state, buf + 2, DEFAULT_POWER_WIDTH,
			...
			/* That failed too, so print an error message and
			 * return "failure": */
			debug_printf ("Error: failed to create power sprite "
					"on line %d of game file \"%s\"\n",
					line, state->gamefile);
			return 1;
		}
		...
		if(power == POWERW || power == POWERS || power == POWERT ||
				power == POWERP)
			GrSetGCForeground(state->gc, GR_COLOR_DARKGREEN);
		else GrSetGCForeground(state->gc, GR_COLOR_DARKRED);
...
int load_game_file(nbstate *state)
{
	...
	while(fgets(buf, 256, fp)) {
		...
		*p = 0; /* Get rid of the newline. */
		...
		/* Compare the line against each of the different keywords: */
			...
			if(lev) {
				debug_printf ("Error: Width must be set "
			...
			if(lev) {
				debug_printf ("Error: Height must be set "
					"before the first level is defined "
					"(see line %d of game file \"%s\")\n",
					line, state->gamefile);
				goto err;
			...
			if(!(state->bats[NORMALBAT] = load_sprite(state,
				...
				GrSetGCForeground(state->gc, GR_COLOR_DARKRED);
			...
			if(state->faderate > 255 || state->faderate < 0) {
				debug_printf ("Invalid fade rate on line "
					"%d of game file \"%s\"\n", line,
					state->gamefile);
				goto err;
			}
			...
			if(strlen(state->cheats[SFCHEAT]) > MAXCHEATLEN) {
				debug_printf ("Cheat sequence too long on "
						"line %d of game file \"%s\"\n",
						line, state->gamefile);
				goto err;
			}
			...
			if(inlevelblock) {
				debug_printf ("Error: BeginLevel while "
			...
			debug_printf ("Error: EndRows without corresponding "
			...
			debug_printf ("Error: EndPowers without "
			...
			if(!inlevelblock) {
				debug_printf ("Error: EndLevel while not in "
			...
			debug_printf ("Unknown command \"%s\" on line %d "
					"of game file \"%s\"\n", buf,
					line, state->gamefile);
		}
	...
	if(ferror(fp)) {
		debug_printf ("Error reading from game file \"%s\" on line "
			"%d\n", state->gamefile, line);
		goto err;
	}
...
parseerr: /* A parse error occured so print an error message: */
	debug_printf ("Parse error on line %d of game file \"%s\"\n", line,
							state->gamefile);
