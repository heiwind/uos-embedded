...
{
	debug_printf ("Usage: nbreaker [ -d game-dir ] [ game-file ]\n");
}
...
static void setup_default_state(nbstate *state)
{
	...
	state->titlesplash = 0;
	state->gamewonsplash = 0;
	state->gamelostsplash = 0;
	state->spritelist = 0;
	state->background = 0;
	...
	state->scores.p = 0;
	state->gc = GrNewGC();
	fid = GrCreateFont((unsigned char*) SCORE_FONT, 0, 0);
...
nbstate *init(int argc, char *argv[])
{
	...
	GrGetScreenInfo(&si);
	if(!si.alphablend) {
		debug_printf ("Error: Nano-X server was built without alpha "
	...
	if(!(state = malloc(sizeof(nbstate)))) return 0;
	...
	state->scores.p = GrNewPixmap(state->canvaswidth, state->scores.h + state->ball.s->h, 0);
/*	state->scores.a = GrNewAlpha(state->canvaswidth, state->scores.h);*/
	state->scores.a = GrNewPixmap(state->canvaswidth, state->scores.h + state->ball.s->h, 0);
	state->canvas = GrNewPixmap(state->canvaswidth, state->canvasheight, 0);
...
/*	state->brickalpha = GrNewAlpha(state->brickwidth, state->brickheight);*/
	state->brickalpha = GrNewPixmap(state->brickwidth, state->brickheight, 0);
	...
	props.title = (unsigned char*) "NanoBreaker";
