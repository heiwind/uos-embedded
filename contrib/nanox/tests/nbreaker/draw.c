...
/* No alpha support. */
#define GrCopyAreaAlpha(canvas, gc, a, x, y, w, h, sprite, fromx, fromy, op) \
	GrCopyArea (canvas, gc, x, y, w, h, sprite, fromx, fromy, 0)
...
void draw_scores(nbstate *state)
{
	...
	draw_background(state, 0, 0, state->canvaswidth, state->scores.h + state->ball.s->h);
	...
	GrCopyAreaAlpha(state->canvas, state->gc, state->scores.a, 0, 0,
			state->canvaswidth, state->scores.h + state->ball.s->h, state->scores.p,
			0, 0, GR_ALPHA_BLEND);
}
