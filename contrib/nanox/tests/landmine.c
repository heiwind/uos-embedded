...
#define	MAXPARAMS	1		/* maximum different game parameters */
...
#define	BUTTONGAP	10		/* mm gap between buttons */
#define	STATUSGAP	15		/* mm gap between buttons and status */

#define	BUTTONWIDTH	100		/* width of buttons (pixels) */
...
nxmain_landmine (void *arg)
{
	...
	font_fixed = GrCreateFont ((unsigned char*) GR_FONT_SYSTEM_FIXED, 0, 0);
	GrGetFontInfo (font_fixed, &fi);
	 ...
	 */
	ROWS = 400;
	COLS = ROWS + RIGHTSIDE + (si.xdpcm * RIGHTGAP / 5);
	mainwid = GrNewWindowEx (GR_WM_PROPS_APPFRAME | GR_WM_PROPS_CAPTION |
		GR_WM_PROPS_CLOSEBOX, (unsigned char*) "Land Mine", GR_ROOT_WINDOW_ID,
		0, 0, COLS, ROWS, GR_COLOR_BLACK);
	...
	boardwid = GrNewWindow(mainwid, x, y, width, height, BOARDBORDER,
		GR_COLOR_NAVYBLUE, GR_COLOR_WHITE);
	/*
	 * Create the buttons.
	 */
	x = rightx;
	...
	buttongc = GrNewGC();
	xorgc = GrNewGC();
	GrSetGCBackground(boardgc, GR_COLOR_NAVYBLUE);
	GrSetGCForeground(cleargc, GR_COLOR_NAVYBLUE);
	GrSetGCForeground(redgc, GR_COLOR_DARKRED);
	GrSetGCForeground(greengc, GR_COLOR_DARKGREEN);
	GrSetGCForeground(statgc, GR_COLOR_GRAY);
	GrSetGCForeground(delaygc, GR_COLOR_BLACK);
	GrSetGCForeground(blackgc, GR_COLOR_BLACK);
...
setcursor(void)
{
	...
	GrSetCursor(boardwid, 9, 12, 4, 6, GR_COLOR_WHITE, GR_COLOR_BLACK,
		fgbits, bgbits);
}
