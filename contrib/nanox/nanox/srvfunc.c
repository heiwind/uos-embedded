...
static int PeekEvent(GR_EVENT *ep);
...
GrQueueLength(void)
{
	...
	SERVER_LOCK();
	for(p = curclient->eventhead; p; p = p->next)
		++count;
...
#if 0 /* was NONETWORK*/
	/* NOTE: select() now called through PeekEvent when bound to server*/
	if (doCheckEvent)
		GsSelect(1L);
#endif
...
void
GsDestroyWindow(GR_WINDOW_ID wid)
{
	GR_WINDOW	*wp;		/* window structure */
	GR_PIXMAP	*pp;
	GR_PIXMAP	*prevpp;
...
{
	SERVER_LOCK();
	GsDestroyRegion(region);
	SERVER_UNLOCK();
}

void
GsDestroyRegion(GR_REGION_ID region)
{
...
{
	SERVER_LOCK();
	GsDestroyFont (fontid);
	SERVER_UNLOCK();
}

void
GsDestroyFont(GR_FONT_ID fontid)
{
	GR_FONT		*fontp;
	GR_FONT		*prevfontp;
...
void
GsDestroyCursor(GR_CURSOR_ID cid)
{
	GR_CURSOR	*cursorp;
 ...
 */
void
GsMoveCursor(GR_COORD x, GR_COORD y)
{
	/*
...
void
GsDestroyTimer (GR_TIMER_ID tid)
{
	GR_TIMER  *timer;
	GR_TIMER  *prev_timer;
...
/**
 * Restart a timer previously created with GrCreateTimer().
 *
 * @param tid the ID of the timer to restart
 */
void
GrRestartTimer (GR_TIMER_ID tid)
{
	GR_TIMER  *timer;
...
void
GsUngrabKey(GR_WINDOW_ID id, GR_KEY key)
{
	GR_GRABBED_KEY *keygrab;
