...
GsAcceptClientFd(int id)
{
	...
	if (connectcount++ > 0) {
		root_client->prev = client;
		client->next = root_client;
	}
	root_client = client;
	return 1;
}
...
GsDestroyClientResources (GR_CLIENT *client)
{
	...
	GR_FONT		*fp, *nfp;
	GR_CURSOR	*cp, *ncp;
	GR_EVENT_CLIENT *ecp, *necp, *pecp = NULL;
	GR_EVENT_LIST	*evp;
	GR_GRABBED_KEY	*kp, *nkp;
	GR_IMAGE	*ip, *nip;
	GR_TIMER	*tp, *ntp;

	/*debug_printf ("Destroy client %d resources\n", client->id);*/
	...
	for(wp=listwp; wp; wp=nwp) {
		...
		while (ecp) {
			...
			if (ecp->client == client) {
				/*debug_printf ( "  Destroy window %d eventclient mask %08lx\n", wp->id, ecp->eventmask);*/
		...
		if (wp->owner == client) {
			/*debug_printf ("  Destroy window %d\n", wp->id);*/
			GsDestroyWindow(wp->id);
		}
	}
	...
	for(pp=listpp; pp; pp=npp) {
		npp = pp->next;
		if (pp->owner == client) {
			/*debug_printf ("  Destroy pixmap %d\n", pp->id);*/
			GsDestroyWindow(pp->id);
		}
	}
	...
	for(rp=listregionp; rp; rp=nrp) {
		nrp = rp->next;
		if (rp->owner == client) {
			/*debug_printf ("  Destroy region %d\n", rp->id);*/
			GsDestroyRegion(rp->id);
		}
	}
	...
	for(tp=list_timer; tp; tp=ntp) {
		ntp = tp->next;
		if (tp->owner == client) {
			/*debug_printf ("  Destroy timer %d\n", tp->id);*/
			GsDestroyTimer(tp->id);
		}
	}
	...
	for(cp=listcursorp; cp; cp=ncp) {
		ncp = cp->next;
		if (cp->owner == client) {
			/*debug_printf ("  Destroy cursor %d\n", cp->id);*/
			GsDestroyCursor(cp->id);
		}
	}
	...
	for (kp=list_grabbed_keys; kp; kp = nkp) {
		...
		if (kp->owner == curclient) {
			/*debug_printf ("  Destroy grabkey %d,%d\n", kp->wid, kp->key);*/
			GsUngrabKey(kp->wid, kp->key);
		}
	}
	/* Free events associated with client*/
	evp = client->eventhead;
	while (evp) {
		/*debug_printf ("  Destroy event %d\n", evp->event.type);*/
