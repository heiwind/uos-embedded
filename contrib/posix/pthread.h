...
#define pthread_mutex_t				lock_t

#define pthread_mutex_lock(lock)		({ lock_take (lock); 0; })
#define pthread_mutex_trylock(lock)		( lock_try (lock) > 0 ? 0 : -1 )
#define pthread_mutex_unlock(lock)		({ lock_release (lock); 0; })
...
#define pthread_cond_signal(cond)		lock_signal (*cond, 0)

#define pthread_cond_broadcast(cond)		lock_signal (*cond, 0)
