...
load_default_cursor( CoreWindowStack *stack )
{
     ...
     FILE            *f;
#else
     static const char bytes[64] = {
		      253,253,253,232,232,232,232,131,211,211,211,64,208,208,208,30,
		      221,221,221,21,235,235,235,16,243,243,243,10,248,248,248,5,
		      251,251,251,2,253,253,253,1,255,255,255,0,255,255,255,0,
...
finish:
     if (f)
          fclose( f );
#else
     /* Use embedded cursor image. */
     memcpy (data, bytes, MIN (64, pitch));
#endif
