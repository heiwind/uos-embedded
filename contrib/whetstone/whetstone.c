...
C	the Whetstone benchmark progra
C
...
int main_whetstone(int argc, const char *argv[])
{
  ...
  unsigned long user_msec;
	...
	while (II < argc) {
		if (strncmp((const unsigned char*)argv[II],(const unsigned char*) "-c", 2) == 0 || argv[II][0] == 'c') {
			continuous = 1;
		} else if (atol((const unsigned char*)argv[II]) > 0) {
			loopstart = atol((const unsigned char*)argv[II]);
		} else {
			debug_printf(USAGE);
			return(1);
		}
...
C
*/
	startmsec = timer_milliseconds (&timer);
...
C
*/
	finimsec = timer_milliseconds (&timer);
...
*/
  ...
  user_msec = finimsec - startmsec;
  float sec = ((float)user_msec) / (1000);

	debug_printf("\n");
	if (sec <= 0.0f) {
		debug_printf("Insufficient duration- Increase the LOOP count\n");
		return(1);
	}

	debug_printf("Loops: %ld, Iterations: %d, Duration: %f sec.\n",
			LOOP, II, sec);

	KIPS = (100.0*LOOP*II)/(float)(sec);
	if (KIPS >= 1000.0)
		debug_printf("C Converted Double Precision Whetstones: %.1f MIPS\n", KIPS/1000.0);
	else
		debug_printf("C Converted Double Precision Whetstones: %.1f KIPS\n", KIPS);
...
{
	debug_printf("%7ld %7ld %7ld %12.4e %12.4e %12.4e %12.4e\n",
						N, J, K, X1, X2, X3, X4);
}
#endif
#endif
