...
#include <mem/mem.h>
mem_pool_t pool;
extern unsigned _mempool_start;
#define NULL 0
/// ----------------------------->
...
int main_linpack(int size)
//void main_linpack(void)
    ...
    mem_init (&pool, (unsigned long)_mempool_start, ((unsigned long)_mempool_start + 0x02000000)); // 32 Мбайта пул памяти SRAM
      
    debug_printf("array size set [%ld]\n",size);
    arsize=size;
    arsize/=2;
    arsize*=2;
    if (arsize<10)
    ...
    memreq=arsize2d*sizeof(REAL)+(long)arsize*sizeof(REAL)+(long)arsize*sizeof(int) + 4;
    debug_printf("Memory required:  %ldK.\n",(memreq+512L)>>10);
    malloc_arg=(size_t)memreq;
    if (malloc_arg!=memreq || (mempool=mem_alloc(&pool, malloc_arg))==NULL)
        {
        debug_printf("Not enough memory available for given array size.\n\n");
        return -1;
        }
    debug_printf("\n\nLINPACK benchmark, %s precision.\n",PREC);
    debug_printf("Machine precision:  %d digits.\n",BASE10DIG);
    debug_printf("Array size %d X %d.\n",arsize,arsize);
    debug_printf("Average rolled and unrolled performance:\n\n");
    debug_printf("    Reps Time(s) DGEFA   DGESL  OVERHEAD    KFLOPS\n");
    debug_printf("----------------------------------------------------\n");
    nreps=1;
...
static REAL linpack(long nreps,int arsize)
    ...
    a=(REAL *)(((unsigned) mempool + 0x7) & ~0x7);
    b=a+arsize2d;
    ipvt=(int *)&b[arsize];
    tdgesl=0;
    tdgefa=0;
...
//    if (totalt<0.01 || (tdgefa+tdgesl)<0.01)
//        return(0.);
   kflops=2.*nreps*ops/(1000.*(tdgefa+tdgesl));
    ...
    if (toverhead<0.)
        toverhead=0.;
    debug_printf("%8ld %6.2f %6.2f%% %6.2f%% %6.2f%%  %9.3f\n",
...
//    return ((REAL)((REAL)clock()/(REAL)CLOCKS_PER_SEC));
      return (REAL)((REAL)timer_milliseconds (&timer)) / ((REAL)1000);
    }
