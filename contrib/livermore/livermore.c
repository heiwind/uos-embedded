...
int main_livermore(int argc, char *argv[])
{
    double pass_time, least, lmult;//, now = 1.0
    double wt;
...
*/    
    ...
    debug_printf("L.L.N.L. 'C' KERNELS: MFLOPS   P.C.  VERSION 4.0\n\n");

     
    debug_printf("Optimisation  %s\n\n",options);
...
//    if (outfile == NULL)
//    {
//        debug_printf(" Cannot open results file LLloops.txt\n\n");
...
//    }
    ...
    // fdebug_printf(outfile   , " #####################################################\n\n");                     
//    for (i=1; i<10; i++)
//    {
//        fdebug_printf(outfile, "%s \n", configdata[i]);
//    }
    // fdebug_printf(outfile   , "\n");
...
//    local_time();
    ...
    // fdebug_printf(outfile   , " #####################################################\n\n");                     
    // fdebug_printf(outfile   , " Livermore Loops Benchmark %s via C/C++ \n", options);

    if (reliability)
    {
        // fdebug_printf(outfile   , " Reliability test %3.0f seconds each loop x 24 x 3\n\n", runSecs);
    }
//    fflush(outfile);    
 ...
 ************************************************************************/
    ...
    debug_printf("Calculating outer loop overhead\n");
    pass = -20;
    extra_loops[0][0] = 1;
    loop = 1000;
    which = 0;
    ...
    do
    {
        ...
        start_clock = timer_milliseconds(&timer); //start_time();
        count = 0;
        count2 = 0;
        pass = pass + 1;        
        do
        ...
        while (count < loop);
//        end_time();
        end_clock = timer_milliseconds(&timer);
//        debug_printf("clock %ld : %ld\n",start_clock, end_clock);
        overhead_l = (double)(end_clock - start_clock)/(CLOCKS_PER_SEC);
//       debug_printf("%10ld times %6.2f seconds\n", loop, overhead_l);
        if (overhead_l > 0.01)
        {
            pass = 0;
        }
    ...
    debug_printf("Overhead for each loop %12.4e seconds\n\n", overhead_l);
 ...
 ************************************************************************/
    for ( section=0 ; section<3 ; section++ )
    {
        ...
        {
 //            local_time();
             // fdebug_printf(outfile   , " Part %ld of 3 start at \n", section + 1);
 //            fflush(outfile);
        }
 ...
 ************************************************************************/
             ...
             debug_printf("Calibrating part %ld of 3\n\n", section + 1);
...
//            start_time();
            start_clock = timer_milliseconds(&timer);
            kernels();

//            end_time();
            end_clock = timer_milliseconds(&timer);
            pass_time = ((double)(end_clock - start_clock))/(CLOCKS_PER_SEC);
            least = 1.0;
            for ( i=1; i<25; i++)
            {
            ...
            debug_printf("Loop count %10ld %5.2f seconds\n", mult, pass_time);
        ...
        debug_printf("Kernel       Floating Pt ops\n");
        debug_printf("No  Passes E No    Total      Secs.  MFLOPS Span     "
                                        "Checksums          OK\n");
        debug_printf("------------ -- ------------- ----- ------- ---- "
                                         "---------------------- --\n");
 ...
 ************************************************************************/
        ...
        {
           debug_printf("\n");
           debug_printf("                     Maximum   Rate%8.2f \n",
                                                  maximum[section]);
           debug_printf("                     Average   Rate%8.2f \n",
                                                  average[section]);
           debug_printf("                     Geometric Mean%8.2f \n",
                                                  geometric[section]);
           debug_printf("                     Harmonic  Mean%8.2f \n",
                                                  harmonic[section]);
           debug_printf("                     Minimum   Rate%8.2f \n\n",
                                                  minimum[section]);
           debug_printf("                     Do Span   %4ld\n\n",
                                                  xspan[section]);
        }        
    }
 ...
 ************************************************************************/
    ...
    debug_printf("                Overall\n\n");
    debug_printf("                Part 1 weight 1\n");
    debug_printf("                Part 2 weight 2\n");
    debug_printf("                Part 3 weight 1\n\n");
    debug_printf("                     Maximum   Rate%8.2f \n", maximum[3]);
    debug_printf("                     Average   Rate%8.2f \n", average[3]);
    debug_printf("                     Geometric Mean%8.2f \n", geometric[3]);
    debug_printf("                     Harmonic  Mean%8.2f \n", harmonic[3]);
...
//               fdebug_printf(outfile, "\n Numeric results were consistent with first\n\n");
            ...
            {
//               fdebug_printf(outfile, " Numeric results were as expected\n\n");
            }
        }
        else
        {
 ...
 ************************************************************************/
    ...
    // fdebug_printf(outfile   , " MFLOPS for 24 loops\n");
    for ( which=1; which<13 ; which++ )
    {
       if (Mflops[0][which] < 10000)
       {
          // fdebug_printf(outfile   , "%7.1f", Mflops[0][which]);
       }
       else
       {
          // fdebug_printf(outfile   , "%7.0f", Mflops[0][which]);
       }
    }
    // fdebug_printf(outfile   , "\n");
    ...
    // fdebug_printf(outfile   , " Overall Ratings\n");
    // fdebug_printf(outfile   , " Maximum Average Geomean Harmean Minimum\n");
    // fdebug_printf(outfile   , "%8.1f%8.1f%8.1f%8.1f%8.1f\n\n",
 ...
 ************************************************************************/
          count2 = 0;    
          end_clock = timer_milliseconds(&timer); //end_time();
          RunTime[section][which] = ((double)(end_clock -start_clock))/(CLOCKS_PER_SEC);
 ...
 ************************************************************************/
             ...
             debug_printf("%2ld %3ld x%4ld %2ld %13.6e %5.2f%8.2f %4ld %22.15e %2ld\n",
             ...
             { 
                 if (compareFail)
                 {
                     debug_printf(" ERRORS have occurred - see log file\n");
                 }
             }
...
*/ 
    ...
    if (reliability)
    {
        ...
        if (count2 == 1)
        {
            ...
            snprintf ((unsigned char*)chek1,SIZEOF_ARRAY(chek1), "%22.15e", Checksum[i][j]);
            snprintf ((unsigned char*)chek2,SIZEOF_ARRAY(chek2), "%22.15e", sumscomp[i][j]);
            if (strcmp ((unsigned char*)chek1, (unsigned char*)chek2)  != 0)
            {
                nsRes = TRUE;
                sumscomp[i][j] = Checksum[i][j];
                debug_printf(" Section %d Test %2d pass %5ld Non-standard result was %s expected %s\n",
        ...
        else
        {
            ...
            if (strcmp ((unsigned char*)chek1, (unsigned char*)chek2)  != 0)
            {
                ...
                if (compareFail == 1)
                {
                   debug_printf(" ERRORS - maximum of 5 reported per loop\n");
                }
                if (failCount < 6)
                {
                   debug_printf(" Section %d Test %2d pass %5ld Different result was %s expected %s\n",
                                       i+1, j, count2, chek1, chek2);
//                   fflush(outfile);
                }
            }
    ...
    else
    {
        for (i=0; i<3; i++)
        {
            ...
            {
                snprintf ((unsigned char*)chek1,SIZEOF_ARRAY(chek1), "%22.15e", Checksum[i][j]);
                snprintf ((unsigned char*)chek2,SIZEOF_ARRAY(chek2), "%22.15e", sumsOut[i][j]);
                if (strcmp ((unsigned char*)chek1, (unsigned char*)chek2)  != 0)
                {
                    errors = errors + 1;
                    debug_printf(" Section %d Test %2d Non-standard result was %s expected %s\n",
                                        i+1, j, chek1, chek2);
                }
            }
        }
        if (errors == 0)
        {
            debug_printf(" Numeric results were as expected\n");
        }
//        fdebug_printf(outfile, "\n");
    }   
}    
