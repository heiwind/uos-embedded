    ...
    )
{
	...
	if (dst == pvPtr->end) {
	    ...
	    pvPtr->next = dst;
	    (*pvPtr->expandProc) (pvPtr, 1, interp->pool);
	    dst = pvPtr->next;
	}
	...
	if (c == termChar) {
	    ...
	    *dst = c;
		...
		pvPtr->next = dst;
		(*pvPtr->expandProc) (pvPtr, length, interp->pool);
		dst = pvPtr->next;
	    }
	    strcpy(dst, value);
	    dst += length;
...
TclParseNestedCmd(
    ...
    , unsigned char **termPtr	/* Store address of terminating character here. */
    ...
    )
{
    ...
    if (shortfall > 0) {
	(*pvPtr->expandProc) (pvPtr, shortfall, interp->pool);
    }
    ...
    )
{
	...
	if (dst == end) {
	    pvPtr->next = dst;
	    (*pvPtr->expandProc) (pvPtr, 20, interp->pool);
	    dst = pvPtr->next;
	    end = pvPtr->end;
	}
	*dst = c;
...
TclParseWords(
    ...
    , unsigned char **termPtr	/* Store address of terminating character here. */
    , int *argcPtr		/* Filled in with actual number of words parsed. */
	...
	mem_pool_t *pool)		/* Get memory from here */
{
    ...
    newbuf = (unsigned char *)mem_alloc (pool, newSpace);

    /*
     * Copy from old buffer to new, free old buffer if needed, and
     * mark new buffer as malloc-ed.
     */
    memcpy ((void*) newbuf, (void*) pvPtr->buffer, pvPtr->next - pvPtr->buffer);
    pvPtr->next = newbuf + (pvPtr->next - pvPtr->buffer);
    if (pvPtr->clientData != 0) {
	mem_free(pvPtr->buffer);
    }
    pvPtr->buffer = newbuf;
