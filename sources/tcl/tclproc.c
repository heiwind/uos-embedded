    ...
    )
{
	...
	argPtr = (Arg*) mem_alloc (interp->pool, (unsigned)
    ...
    )
{
    ...
    iPtr = procPtr->iPtr;
    Tcl_InitHashTable (&frame.varTable, interp->pool, TCL_STRING_KEYS);
    if (iPtr->varFramePtr != 0) {
	...
	if ((argPtr->nextPtr == 0) &&
	  (strcmp (argPtr->name, (unsigned char*) "args") == 0)) {
	    ...
	    value = Tcl_Merge (interp->pool, argc, args);
