 ...
 */
void
gost_init (gost_t *g, const unsigned char k1[16], const unsigned char k2[16],
	const unsigned char k3[16], const unsigned char k4[16],
	const unsigned char k5[16], const unsigned char k6[16],
	const unsigned char k7[16], const unsigned char k8[16])
{
	int i;
 ...
 */
static word32_t
f (gost_t *g, word32_t x)
{
	/* Do substitutions */
	x = g->k87 [x>>24 & 255] << 24 | g->k65 [x>>16 & 255] << 16 |
	    g->k43 [x>> 8 & 255] <<  8 | g->k21 [x & 255];
 ...
 */
void
gost_crypt (gost_t *g, word32_t const in[2], word32_t out[2],
	word32_t const key[8])
{
	word32_t n1, n2;	/* As named in the GOST */
 ...
 */
void
gost_decrypt (gost_t *g, word32_t const in[2], word32_t out[2],
	word32_t const key[8])
{
	word32_t n1, n2;	/* As named in the GOST */
 ...
 */
void
gost_create_gamma (gost_t *g, word32_t *gamma, word32_t const synchro[2],
	word32_t const key[8])
{
        int i;
        word32_t temp[2];
...
#define C2 0x01010101
	...
	gost_crypt (g, synchro, temp, key);
		...
		gost_crypt (g, temp, &gamma[i*2], key);
         }
}
 ...
 * y[i] = x[i] ^ gost_crypt (y[i-1])
 * x[i] = y[i] ^ gost_crypt (y[i-1])
 * Where y[-1] is the IV.
 *
 * The IV is modified in place.  Again, len is in *blocks*.
 */
void
gost_cfb_encrypt (gost_t *g, word32_t const *in, word32_t *out, int len,
	word32_t iv[2], word32_t const key[8])
{
	while (len-- > 0) {
		gost_crypt (g, iv, iv, key);
		iv[0] = *out++ ^= iv[0];
		iv[1] = *out++ ^= iv[1];
	}
}
...
void
gost_cfb_decrypt (gost_t *g, word32_t const *in, word32_t *out, int len,
	word32_t iv[2], word32_t const key[8])
{
	word32_t t;
 ...
 */
void
gost_mac (gost_t *g, word32_t const *in, int len, word32_t out[2],
	word32_t const key[8])
{
	word32_t n1, n2;	/* As named in the GOST */
...
#define RAND32 ((word32_t)rand() << 17 ^ (word32_t)rand() << 9 ^ rand())
...
main (void)
{
	...
	gost_init (&gost, k1, k2, k3, k4, k5, k6, k7, k8);
	...
	for (i = 0; i < 1000; i++) {
		...
		gost_crypt (&gost, plain, cipher, key);
		for (j = 0; j < 99; j++)
			gost_crypt (&gost, cipher, cipher, key);
		for (j = 0; j < 100; j++)
			gost_decrypt (&gost, cipher, cipher, key);
