...
typedef	struct TX_BD_ctrl
{
	...
	unsigned	PAD_DIS		: 1;	// Отключение дополнения пакетов длиной менее MinFrame до минимальной длины PAD-ами (1-дополнение не производится, дополнения производится)
	unsigned	CRC_DIS		: 1;	// Отключение дополнения пакетов полем CRC32
	volatile unsigned RTRY		: 4;	// Счетчик кол-ва попыток передачи исходящего пакета
...
typedef	struct bits_MAC_CTRL
{
	...
	unsigned PRO_EN		: 1;	// Включение приема всех пакетов независимо от их МАС-адреса (активный уровень "1")
...
typedef	struct bits_PHY_CTRL
{
	...
	unsigned PHY_DIR		:1;	// Порядок передачи битов в полубайте (1-прямой,0-инверсный)
	...
	unsigned PHY_RXEN		:1;	// Разрешение работы ПРЬ встроенного контроллера PHY-уровня (активный уровень "1")
	unsigned PHY_TXEN		:1;	// Разрешение работы ПРД встроенного контроллера PHY-уровня (активный уровень "1")
	unsigned PHY_EXT_EN		:1;	// Переключение на работу с внешним кнтроллером PHY-уровня (активный уровень "1")
	unsigned PHY_RST		:1;	// Сброс встроенного контроллера контроллера PHY-уровня (активный уровень "1")
#else
	unsigned PHY_RST		:1;	// Сброс встроенного контроллера контроллера PHY-уровня (активный уровень "1")
	unsigned PHY_EXT_EN		:1;	// Переключение на работу с внешним кнтроллером PHY-уровня (активный уровень "1")
	unsigned PHY_TXEN		:1;	// Разрешение работы ПРД встроенного контроллера PHY-уровня (активный уровень "1")
