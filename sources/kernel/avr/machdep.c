...
avr_task_switch (task_t *target)
{
	...
	task_current->stack_context = inw (SP);
//debug_printf ("avr_switch %p/%S ->", task_current, task_current->name);

	task_current = target;

//debug_printf (" %p/%S\n", task_currentm, task_current->name);
	/* Switch to the new task. */
	outw (task_current->stack_context, SP);
 ...
 */
void
avr_intr_allow (unsigned char irq)
{
	switch (irq) {
#if defined (__AVR_ATmega103__) || defined (__AVR_ATmega128__)
 ...
 */
void
avr_build_stack_frame (task_t *t, unsigned short func, unsigned short arg,
	char *sp)
{
	...
	*sp-- = 0;			/* r27 */
	*sp-- = 0;			/* r26 */
	*sp-- = arg >> 8;		/* r25 - task argument high... */
	*sp-- = arg;			/* r24 ...low */
	*sp-- = 0;			/* r23 */
	*sp-- = 0;			/* r22 */
	*sp-- = 0;			/* r21 */
